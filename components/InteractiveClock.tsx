...
function FlipTile({ digit, color }: { digit: string; color: string }) {
  return (
    <div className="relative w-[8.5rem] h-[12rem] bg-gradient-to-b from-slate-900 to-[#030712] border border-slate-700/50 rounded-2xl flex items-center justify-center shadow-[0_16px_60px_rgba(0,0,0,0.8)] overflow-hidden">
      {/* top sheen */}
      ...
      <AnimatePresence mode="popLayout">
        <motion.span
          ...
          className={`z-10 font-mono font-black text-[8rem] tabular-nums leading-none select-none ${color}`}
        >
          {digit}
        </motion.span>
      </AnimatePresence>
...
function FlipPanel({ value, label, color }: { value: string; label: string; color: string }) {
  return (
    <div className="flex flex-col items-center gap-3 select-none">
      <span className="text-[10px] font-black tracking-[0.5em] text-slate-600 uppercase">{label}</span>
...
export function InteractiveClock() {
  ...
  return (
    <>
      {/* ── LEFT: Hours ── bring inward with left-20 */}
      <div className="absolute left-20 top-[44%] -translate-y-1/2 z-20 hidden md:flex flex-col items-center">
        <FlipPanel value={hoursStr} label="hours" color="text-amber-400" />
      </div>

      {/* ── RIGHT: Minutes ── bring inward with right-20 */}
      <div className="absolute right-20 top-[44%] -translate-y-1/2 z-20 hidden md:flex flex-col items-center">
